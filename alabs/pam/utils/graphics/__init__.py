...
def xywh_to_x1y1x2y2(coord):
    """
    x, y, width, height 의 값을 x1, y1, x2, y2 로 변경
    >>> a = (10, 20, 44, 23)
    >>> b = xywh_to_x1y1x2y2(a)
    >>> assert b == [10, 20, 54, 43]
...
# 이미지에 그리기 기능 모음
################################################################################
def draw_rectangle(image_path: str, locations: (tuple, list),
                   rgb: (tuple, list), thickness: int):
    """

    :param image_path: file path
    :param locations: (1, 2, 3, 4)
        x1, y1, x2, y2
    :param rgb: (0, 0, 255)
        r, g, b
    :param thickness: 1
    ...
    """
    if 4 > len(locations):
        raise ValueError("Locations value error. 4 number are required.")
...
################################################################################
def draw_put_text(image_path: str, value, locations: (tuple, list),
                  rgb: (tuple, list), font='SIMPLEX',
    ...
    font = getattr(cv2, font_type[font])
    if 2 > len(locations):
        raise ValueError("Locations value error. 2 number are required.")

    img = cv2.imread(image_path)
    cv2.putText(img, value, locations, font, font_scale, rgb, thickness)
    cv2.imwrite(image_path, img)


def draw_box_with_title(image_path, title, locations,
                        t_rgb=RGB.RED1.value, b_rgb=RGB.BLUE.value, font='SIMPLEX',
                        font_scale=0.4, box_thickness=1, font_tickness=1):
    draw_rectangle(image_path, locations, b_rgb, box_thickness)
    draw_put_text(image_path, title, (locations[0], locations[1]-2), t_rgb,
                  font, font_scale, font_tickness)